...
import { Download, FileText, FileSpreadsheet, FileDown, X, UserCheck, ChevronDown } from "lucide-react";
...
export const ExportMenu: React.FC<ExportMenuProps> = ({
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text(currentCompany?.name || "Report", 14, y);
      y += 7;
      ...
      if (currentCompany?.vatNumber) {
        ...
        doc.text(`VAT: ${currentCompany.vatNumber}  |  CR: ${currentCompany.crNumber || ""}  |  ${currentCompany.city || ""}`, 14, y);
        y += 5;
      }

      doc.setFontSize(9);
      doc.setTextColor(100);
      doc.text(`${sanitize(filename).replace(/_/g, " ").toUpperCase()}   |   Generated: ${new Date().toLocaleDateString()}`, 14, y);
      y += 3;
      ...
      sections.forEach(sec => {
        ...
        autoTable(doc, {
          ...
          margin: { top: 15, bottom: selectedSig ? 45 : 25 },
          rowPageBreak: "avoid",
        });
      ...
      if (selectedSig) {
        ...
        doc.line(14, y + 12, 70, y + 12);
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(50);
        doc.text(selectedSig.name, 14, y + 17);
        doc.setFont("helvetica", "normal");
        doc.text(selectedSig.designation || "", 14, y + 21);
      }
          ...
          <div className="relative w-80 bg-white h-full shadow-2xl flex flex-col overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{language === "ar" ? "اختر التنسيق والإعدادات" : "Choose format & settings"}</p>
              </div>
            ...
            <div className="flex-1 p-5 space-y-6">
              {/* Branding options */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-brand-primary mb-3">{language === "ar" ? "إعدادات الوثيقة" : "Document Branding"}</p>
                <div className="space-y-2">
                  {[
                    { key: "logo", label: language === "ar" ? "تضمين الشعار" : "Include Logo", val: includeLogo, set: setIncludeLogo },
                    { key: "letterhead", label: language === "ar" ? "ورق الرسائل" : "Letterhead", val: includeLetterhead, set: setIncludeLetterhead },
                    { key: "stamp", label: language === "ar" ? "الختم الرسمي" : "Official Stamp", val: includeStamp, set: setIncludeStamp },
                  ].map(opt => (
                    <label key={opt.key} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 cursor-pointer hover:border-brand-primary transition-colors">
                      <span className="text-xs font-semibold text-slate-700">{opt.label}</span>
                      <input type="checkbox" checked={opt.val} onChange={e => opt.set(e.target.checked)} className="rounded border-slate-300 text-brand-primary" />
                    </label>
                  ))}
                  ...
                  <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-semibold text-slate-700">{language === "ar" ? "المفوض بالتوقيع" : "Authorized Signatory"}</span>
                    </div>
                    <select value={selectedSigId} onChange={e => setSelectedSigId(e.target.value)}
                      className="w-full text-xs border border-slate-200 rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-brand-primary bg-white">
                    ...
                    {signatories.length === 0 && (
                      <p className="text-[10px] text-slate-400 mt-1">{language === "ar" ? "أضف مفوضين من صفحة الإعدادات" : "Add signatories in Settings page"}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {/* Export buttons */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600 mb-3">{language === "ar" ? "تنسيق الملف" : "Output Format"}</p>
                <div className="space-y-2">
