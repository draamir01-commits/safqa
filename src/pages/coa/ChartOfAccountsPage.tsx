...
import { Plus, Trash2, ShieldAlert, Printer, FileText, X} from "lucide-react";
    ...
    <div className="flex flex-col gap-6 font-sans">
      <div className="flex items-center justify-between">
        <div>
          ...
          <ExportMenu data={accounts} filename="chart-of-accounts" headers={{ code: "Code", name: "Name", nameAr: "Arabic Name", type: "Type", balance: "Balance" }} />
          <button
          ...
          <div className="relative w-80 bg-white h-full shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{language === "ar" ? "دليل الحسابات" : "Chart of Accounts"}</p>
              </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    let footerHTML = `<div style="position:fixed;bottom:0;left:0;width:100%;background:#fff;border-top:0.5px solid #e8ecf0;padding:5px 12mm;display:flex;justify-content:space-between;font-size:7pt;color:#888;z-index:10"><span>${co?.name||""}</span><span>Chart of Accounts</span><span>${new Date().toLocaleDateString()}</span></div>`;
                    ...
                    const rows = (accounts as any[]).map((row: any, i: number) => {
                      const cells = [String(row.code||""), String(row.name||""), String(row.nameAr||""), String(row.type||""), String(row.balance||"")];
                    ...
                    const html = [
                      ...
                      "<span style='font-size:18pt;font-weight:800'>Chart of Accounts</span>",
                      "</div>",
                      "<div style='border-top:2px solid #e2e8f0;margin-bottom:12px'></div>",
                      "<table style='width:100%;border-collapse:collapse;margin-bottom:12px'>",
                      "<thead><tr style='background:#2d3748;color:#fff'>",
                      "<th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Code</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Name</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Arabic Name</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Type</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Balance</th>",
                      "</tr></thead>",
                      `<tbody>${rows}</tbody>`,
                      "</table>",
                      sigHTML,
