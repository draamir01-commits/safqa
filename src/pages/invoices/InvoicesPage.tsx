...
export const InvoicesPage: React.FC = () => {
  ...
  const exportInvoicePDF = async (inv: Invoice, qrUrl: string, opts?: { lhMode: string; lhId: string; logo: boolean; stamp: boolean; sigId: string; includeSig: boolean; }) => {
    ...
    const isSub = inv.type === "simplified";
    const titleEN = isSub ? "Simplified Tax Invoice" : "Tax Invoice";
    const titleAR = isSub ? "فاتورة ضريبية مبسطة" : "فاتورة ضريبية";
    ...
    const lhList = expLetterheads.length > 0 ? expLetterheads
      : [{ id: "primary", url: co?.fullLetterhead || "" }, ...((co?.additionalLetterheads || []) as any[])];
    const selLH = lhList.find((l: any) => l.id === o.lhId) || lhList[0];
    const lhImgUrl: string = (selLH?.url || co?.fullLetterhead || "") as string;
    const headerImgUrl: string = (co?.headerAsset || co?.letterheadHeader || "") as string;
    const footerImgUrl: string = (co?.footerAsset || co?.letterheadFooter || "") as string;
    const sigObj: any = expSignatories.find((s: any) => s.id === o.sigId) || null;
    ...
    const rightLines = [co?.addressAr||co?.address||"",
      ...
      .filter(Boolean).map((l: string) => `<div style="font-family:Cairo,Arial,sans-serif;font-size:7.5pt;color:#444;line-height:1.7;direction:rtl">${l}</div>`).join("");
    ...
    if (o.lhMode === "full" && lhImgUrl) {
      headerHTML = `<div style="position:fixed;top:0;left:0;width:100%;height:100%;z-index:-1"><img src="${lhImgUrl}" style="width:100%;height:100%;object-fit:fill"/></div>`;
      padTop = "48mm";
    } else if (o.lhMode === "header") {
      // Use headerAsset if available, otherwise fallback to lhImgUrl
      const hUrl = headerImgUrl || lhImgUrl;
      if (hUrl) {
      ...
      headerHTML = `<table style="width:100%;border-bottom:2px solid #e2e8f0;margin-bottom:10px;border-collapse:collapse">
  <tr>
    ...
    <td style="width:38%;text-align:right;vertical-align:top;padding-bottom:10px">
      <div style="font-family:Cairo,Arial,sans-serif;font-size:11pt;font-weight:700;direction:rtl;margin-bottom:4px">${co?.nameAr||""}</div>
      ${rightLines}
    </td>
  </tr>
</table>`;
    ...
    // Footer HTML
    const pgNum = `<div style="display:flex;justify-content:space-between;font-size:7pt;color:#888"><span>${co?.name||""}</span><span>Page 1 of 1 - ${invNum}</span><span>${invNum}</span></div>`;
    let footerHTML = "";
    if (o.lhMode === "header" && footerImgUrl) {
      footerHTML = `<div style="position:fixed;bottom:0;left:0;width:100%;z-index:5;background:#fff">
  <div style="padding:4px 0;border-top:0.5px solid #e8ecf0">${pgNum}</div>
  <div style="line-height:0"><img src="${footerImgUrl}" style="width:100%;max-height:25mm;object-fit:cover;display:block"/></div>
</div>`;
      padBot = "32mm";
    } else {
      footerHTML = `<div style="position:fixed;bottom:0;left:0;width:100%;background:#fff;border-top:0.5px solid #e8ecf0;padding:5px 0 6px;z-index:10">${pgNum}</div>`;
    }

    // Info row builder
    const IR = (le: string, ve: string, la: string, va: string): string => (!ve && !va) ? "" :
      `<tr><td style="font-weight:700;padding:5px 8px;width:95px;background:#f8fafc;border:0.5px solid #c8cdd5;font-size:8pt">${le}</td><td style="padding:5px 8px;border:0.5px solid #c8cdd5;font-size:8pt;min-width:110px">${ve||""}</td><td style="font-family:Cairo,Arial,sans-serif;font-weight:700;padding:5px 8px;width:110px;background:#f8fafc;border:0.5px solid #c8cdd5;font-size:8pt;text-align:right;direction:rtl">${la}</td><td style="font-family:Cairo,Arial,sans-serif;padding:5px 8px;border:0.5px solid #c8cdd5;font-size:8pt;text-align:right;direction:rtl;min-width:110px">${va||""}</td></tr>`;
    ...
    const lineRows = (inv.lineItems||[]).map((l: any, i: number) => {
      ...
      return `<tr style="background:${i%2===0?"#fff":"#f8fafc"}">
        ...
        <td style="text-align:right;padding:6px 5px;border:0.5px solid #cbd5e0;font-size:8pt">${(l.vatAmount||0).toFixed(2)} <span style="font-size:7pt;color:#666">${l.vatRate||15}%</span></td>
...
<div style="margin-top:20px;padding-top:10px;border-top:1px solid #e2e8f0;display:flex;align-items:flex-end;justify-content:space-between;gap:16px">
  <div style="flex:1">
    <div style="font-size:9pt;font-weight:700;margin-bottom:12px">Authorized Signatory / <span style="font-family:Cairo,Arial,sans-serif">المفوض بالتوقيع</span></div>
    ${sigObj && o.includeSig && sigObj.signatureUrl ? `<img src="${sigObj.signatureUrl}" style="height:36px;max-width:100px;object-fit:contain;display:block;margin-bottom:4px"/>` : `<div style="height:36px"></div>`}
    <div style="border-bottom:1.5px solid #333;width:160px;margin-bottom:5px"></div>
    <div style="font-size:9.5pt;font-weight:700">${sigObj ? sigObj.name : ""}</div>
    <div style="font-size:8pt;color:#555;margin-top:2px">${sigObj ? (sigObj.designation||"") : ""}</div>
  </div>
  ${o.stamp && co?.stamp ? `<div style="flex-shrink:0;text-align:center"><img src="${co.stamp}" style="width:90px;height:90px;object-fit:contain;display:block"/><div style="font-size:7pt;color:#888;margin-top:4px">Company Stamp / <span style="font-family:Cairo,Arial,sans-serif">ختم الشركة</span></div></div>` : ""}
</div>` : "";
    ...
    const html = [
      ...
      "</tr></thead>",
      `<tbody>${lineRows||"<tr><td colspan=\"7\" style=\"text-align:center;padding:12px;color:#999\">No items</td></tr>"}</tbody>`,
      "</table>",
