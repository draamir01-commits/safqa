...
import { InvoicePrintDialog, InvoicePrintOptions } from "../../components/ui/InvoicePrintDialog";
import { ZatcaInvoicePdf } from "../../components/ui/ZatcaInvoicePdf";
...
export const InvoicesPage: React.FC = () => {
  ...
  const [printOptions, setPrintOptions] = React.useState<InvoicePrintOptions | null>(null);
  ...
  const exportInvoicePDF = async (inv: Invoice, qrUrl: string, opts?: { lhMode: string; lhId: string; logo: boolean; stamp: boolean; sigId: string; includeSig: boolean; }) => {
    ...
    // Inline styles — everything explicit so no CSS class issues
    const S = {
      page:    "font-family:Cairo,Arial,sans-serif;font-size:9pt;color:#1a1a1a;background:#fff;padding:12mm;",
      ar:      "font-family:Cairo,Arial,sans-serif;direction:rtl;display:inline-block;",
      arBlock: "font-family:Cairo,Arial,sans-serif;direction:rtl;text-align:right;",
    };

    // Company left lines
    const leftLines = [co?.address||"", co?.city ? co.city+", Kingdom of Saudi Arabia":"", co?.email||"", co?.phone||"",
    ...
    const rightLines = [co?.addressAr||co?.address||"",
      ...
      .filter(Boolean).map(l=>`<div style="font-family:Cairo,Arial,sans-serif;font-size:7.5pt;color:#444;line-height:1.7;direction:rtl">${l}</div>`).join("");

    const logoHtml = co?.logo
      ? `<img src="${co.logo}" style="max-height:55px;max-width:110px;object-fit:contain;display:block;margin:0 auto"/>`
      : `<div style="width:55px;height:55px;background:#1d4ed8;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:20pt;font-weight:700;margin:0 auto">${(co?.nameAr||co?.name||"S")[0]}</div>`;

    // Info grid row
    const IR = (len:string, ven:string, lar:string, var_:string) => !ven && !var_ ? "" :
      `<tr>
        <td style="font-weight:700;padding:5px 8px;width:95px;background:#f8fafc;border:0.5px solid #c8cdd5;font-size:8pt;color:#333">${len}</td>
        <td style="padding:5px 8px;border:0.5px solid #c8cdd5;font-size:8pt;color:#1a1a1a;min-width:110px">${ven||""}</td>
        <td style="font-family:Cairo,Arial,sans-serif;font-weight:700;padding:5px 8px;width:110px;background:#f8fafc;border:0.5px solid #c8cdd5;font-size:8pt;color:#333;text-align:right;direction:rtl" class="notranslate" lang="ar">${lar}</td>
        <td style="font-family:Cairo,Arial,sans-serif;padding:5px 8px;border:0.5px solid #c8cdd5;font-size:8pt;color:#1a1a1a;text-align:right;direction:rtl;min-width:110px" class="notranslate" lang="ar">${var_||""}</td>
      </tr>`;
    ...
    const lineRows = (inv.lineItems||[]).map((l:any,i:number)=>{
      ...
      return `<tr style="background:${i%2===0?"#fff":"#f8fafc"}">
        ...
        <td style="padding:6px 7px;border:0.5px solid #cbd5e0;font-size:8pt">${l.name||""}${l.nameAr?`<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#666;direction:rtl" class="notranslate" lang="ar">${l.nameAr}</span>`:""}</td>
    ...
    const lhMode = o.lhMode;
    const selLH = expLetterheads.find((l: any) => l.id === o.lhId) || expLetterheads[0];
    const lhImgUrl = selLH?.url || (currentCompany as any)?.fullLetterhead || "";
    const footerImgUrl = (currentCompany as any)?.footerAsset || (currentCompany as any)?.letterheadFooter || "";

    // Resolve signatory
    const sigObj = expSignatories.find((s: any) => s.id === o.sigId) || null;

    // Build header HTML based on letterhead mode
    const buildHeaderHtml = () => {
      if (lhMode === "full" && lhImgUrl) {
        return `
<!-- FULL PAGE LETTERHEAD -->
<div style="position:fixed;top:0;left:0;width:100%;height:100%;z-index:-1;pointer-events:none">
  <img src="${lhImgUrl}" style="width:100%;height:100%;object-fit:fill;display:block" alt="letterhead"/>
</div>
<div style="height:48mm"></div>`;
      } else if (lhMode === "header" && lhImgUrl) {
        return `
<!-- HEADER BANNER -->
<div style="margin:-12mm -12mm 10px -12mm;line-height:0">
  <img src="${lhImgUrl}" style="width:calc(100% + 24mm);max-height:50mm;object-fit:cover;display:block" alt="header"/>
</div>`;
      } else {
        // Default: bilingual text header with logo
        return `
<!-- COMPANY TEXT HEADER -->
<table style="width:100%;border-bottom:2px solid #e2e8f0;padding-bottom:10px;margin-bottom:10px;border-collapse:collapse">
  <tr>
    ...
    <td style="width:24%;text-align:center;vertical-align:middle;padding:0 8px">
      ${o.logo && co?.logo ? `<img src="${co.logo}" style="max-height:55px;max-width:110px;object-fit:contain;display:block;margin:0 auto"/>` : logoHtml}
    </td>
    ...
    const buildFooterHtml = () => {
      if (lhMode === "header" && footerImgUrl) {
        return `
<!-- FOOTER BANNER + PAGE NUM -->
<div style="position:fixed;bottom:0;left:0;width:100%;z-index:10;line-height:0">
  <div style="background:#fff;padding:4px 12mm;display:flex;justify-content:space-between;font-size:7pt;color:#888;border-top:0.5px solid #e8ecf0">
    <span>${co?.name||""}</span><span>Page 1 of 1 - ${invNum}</span><span>${invNum}</span>
  </div>
  <img src="${footerImgUrl}" style="width:100%;max-height:25mm;object-fit:cover;display:block" alt="footer"/>
</div>
<div style="height:30mm"></div>`;
      } else if (lhMode === "full") {
        return `
<div style="position:fixed;bottom:0;left:0;width:100%;background:transparent;padding:4px 12mm;display:flex;justify-content:space-between;font-size:7pt;color:#555;z-index:10">
  <span>${co?.name||""}</span><span>Page 1 of 1 - ${invNum}</span><span>${invNum}</span>
</div>
<div style="height:16px"></div>`;
        ...
        return `
<!-- PAGE FOOTER -->
<div style="position:fixed;bottom:0;left:0;width:100%;background:#fff;padding:5px 12mm 6px;border-top:0.5px solid #e8ecf0;display:flex;justify-content:space-between;font-size:7pt;color:#888;z-index:10">
  <span>${co?.name||""}</span><span>Page 1 of 1 - ${invNum}</span><span>${invNum}</span>
</div>
<div style="height:16px"></div>`;
      }
    ...
    // Build signatory+stamp HTML
    const buildSigHtml = () => {
      const hasSig = sigObj && o.sigId;
      const hasStamp = o.stamp && co?.stamp;
      if (!hasSig && !hasStamp) return "";
      return `
<div style="margin-top:20px;padding-top:10px;border-top:1px solid #e2e8f0;display:flex;align-items:flex-end;justify-content:space-between;gap:16px">
  <div style="flex:1;min-width:0">
    <div style="font-size:9pt;font-weight:700;color:#1a1a1a;margin-bottom:12px">Authorized Signatory / <span style="font-family:Cairo,Arial,sans-serif" class="notranslate" lang="ar">المفوض بالتوقيع</span></div>
    ${hasSig && o.includeSig && (sigObj as any)?.signatureUrl ? `<img src="${(sigObj as any).signatureUrl}" style="height:36px;max-width:100px;object-fit:contain;display:block;margin-bottom:4px"/>` : `<div style="height:36px"></div>`}
    <div style="border-bottom:1.5px solid #333;width:160px;margin-bottom:5px"></div>
    <div style="font-size:9.5pt;font-weight:700;color:#1a1a1a">${hasSig ? (sigObj as any).name : ""}</div>
    <div style="font-size:8pt;color:#555;margin-top:2px">${hasSig ? ((sigObj as any).designation||"") : ""}</div>
  </div>
  ${hasStamp ? `<div style="flex-shrink:0;text-align:center;padding-left:16px"><img src="${co.stamp}" style="width:90px;height:90px;object-fit:contain;display:block"/><div style="font-size:7pt;color:#888;margin-top:4px">Company Stamp / <span style="font-family:Cairo,Arial,sans-serif" class="notranslate" lang="ar">ختم الشركة</span></div></div>` : ""}
</div>`;
    };
...
<!-- TITLE -->
<table style="width:100%;margin:8px 0 4px">
  <tr>
    <td style="text-align:center">
      <span style="font-size:18pt;font-weight:800;margin-right:24px">${titleEN}</span>
      <span style="font-family:Cairo,Arial,sans-serif;font-size:18pt;font-weight:800;direction:rtl" class="notranslate" lang="ar">${titleAR}</span>
    </td>
  </tr>
</table>
...
<table style="width:100%;border-collapse:collapse;margin-bottom:10px">
  <thead>
    <tr style="background:#2d3748;color:#fff">
      ...
      <th style="padding:6px 7px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt">
        Description<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#e2e8f0;direction:rtl" class="notranslate" lang="ar">الوصف</span>
      </th>
      <th style="padding:6px 5px;text-align:center;border:0.5px solid #4a5568;font-size:7.5pt;width:44px">
        Qty<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#e2e8f0;direction:rtl" class="notranslate" lang="ar">الكمية</span>
      </th>
      <th style="padding:6px 5px;text-align:right;border:0.5px solid #4a5568;font-size:7.5pt;width:58px">
        Price<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#e2e8f0;direction:rtl" class="notranslate" lang="ar">السعر</span>
      </th>
      <th style="padding:6px 5px;text-align:right;border:0.5px solid #4a5568;font-size:7.5pt;width:70px">
        Taxable amount<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#e2e8f0;direction:rtl" class="notranslate" lang="ar">المبلغ الخاضع</span>
      </th>
      <th style="padding:6px 5px;text-align:right;border:0.5px solid #4a5568;font-size:7.5pt;width:62px">
        VAT amount<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#e2e8f0;direction:rtl" class="notranslate" lang="ar">القيمة المضافة</span>
      </th>
      <th style="padding:6px 5px;text-align:right;border:0.5px solid #4a5568;font-size:7.5pt;width:68px">
        Line amount<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7pt;color:#e2e8f0;direction:rtl" class="notranslate" lang="ar">المجموع</span>
      </th>
    </tr>
  </thead>
  <tbody>
    ${lineRows||`<tr><td colspan="7" style="text-align:center;padding:12px;color:#999;font-size:8pt">No items</td></tr>`}
  </tbody>
</table>

<!-- BOTTOM: QR + TOTALS side by side -->
  ...
  <tr>
    <td style="width:50%;vertical-align:top;padding-right:12px">
      ...
      <div style="font-size:6.5pt;color:#555;margin-top:5px;max-width:200px;line-height:1.5">
        ...
        <span style="font-family:Cairo,Arial,sans-serif;direction:rtl;display:block;margin-top:3px" class="notranslate" lang="ar">تم ترميز هذا الرمز وفقاً لمتطلبات هيئة الزكاة والضريبة والجمارك للفوترة الإلكترونية</span>
      </div>
    </td>
    <td style="width:50%;vertical-align:top">
      <table style="width:100%;border-collapse:collapse;border:0.5px solid #c8cdd5">
        <tr>
          <td style="padding:6px 10px;border:0.5px solid #c8cdd5;font-size:8pt;font-weight:600">
            Subtotal<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7.5pt;color:#555;direction:rtl;display:block" class="notranslate" lang="ar">المجموع الفرعي</span>
          </td>
          <td style="padding:6px 10px;border:0.5px solid #c8cdd5;font-size:8pt;font-weight:600;text-align:right">${(inv.subtotal||0).toFixed(2)} ریال</td>
        </tr>
        <tr>
          <td style="padding:6px 10px;border:0.5px solid #c8cdd5;font-size:8pt;font-weight:600">
            Total VAT<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7.5pt;color:#555;direction:rtl;display:block" class="notranslate" lang="ar">إجمالي ضريبة القيمة المضافة</span>
          </td>
          <td style="padding:6px 10px;border:0.5px solid #c8cdd5;font-size:8pt;font-weight:600;text-align:right">${(inv.totalVat||0).toFixed(2)} ریال</td>
        </tr>
        <tr style="background:#1e2d3d">
          <td style="padding:6px 10px;border:0.5px solid #2d3e4f;font-size:8pt;font-weight:700;color:#fff">
            Total<br><span style="font-family:Cairo,Arial,sans-serif;font-size:7.5pt;color:#ccc;direction:rtl;display:block" class="notranslate" lang="ar">المجموع شامل القيمة المضافة</span>
          </td>
          <td style="padding:6px 10px;border:0.5px solid #2d3e4f;font-size:8pt;font-weight:700;color:#fff;text-align:right">${(inv.grandTotal||0).toFixed(2)} ریال</td>
        </tr>
      </table>
    </td>
  </tr>
...
window.onload=function(){setTimeout(function(){window.print()},1200)};
  ...
  const handleConfirmPrint = (opts: InvoicePrintOptions) => {
  ...
  const handleExecutePrint = () => {
    ...
    style.innerHTML = `@media print { body * { visibility: hidden; } #invoice-print-root, #invoice-print-root * { visibility: visible; } #invoice-print-root { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: white !important; z-index: 99999; overflow: visible !important; padding: 0 !important; margin: 0 !important; box-shadow: none !important; border-radius: 0 !important; } .no-print { display: none !important; } @page { margin: 8mm; size: A4; } }`;
    document.head.appendChild(style);
    const prevTitle = document.title;
    document.title = selectedInvoice?.invoiceNumber || "Invoice";
    window.print();
            ...
            <div className="flex items-center justify-between border-b pb-3 no-print">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={handlePrint} variant="success" size="sm" className="flex items-center gap-2 font-bold px-4">
                  ...
                  {language === "ar" ? "طباعة" : "Print / Save PDF"}
                </Button>
              </div>
            </div>
