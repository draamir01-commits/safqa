...
import { Plus, Trash2, ShieldAlert, Pencil, Printer, FileText, X} from "lucide-react";
          ...
          <div className="relative w-80 bg-white h-full shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{language === "ar" ? "المنتجات والخدمات" : "Products & Services"}</p>
              </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    let footerHTML = `<div style="position:fixed;bottom:0;left:0;width:100%;background:#fff;border-top:0.5px solid #e8ecf0;padding:5px 12mm;display:flex;justify-content:space-between;font-size:7pt;color:#888;z-index:10"><span>${co?.name||""}</span><span>Products & Services</span><span>${new Date().toLocaleDateString()}</span></div>`;
                    ...
                    const rows = (products as any[]).map((row: any, i: number) => {
                      const cells = [String(row.name||""), String(row.nameAr||""), String(row.category||""), String(row.salePrice||""), String(row.unit||"")];
                    ...
                    const html = [
                      ...
                      "<span style='font-size:18pt;font-weight:800'>Products & Services</span>",
                      "</div>",
                      "<div style='border-top:2px solid #e2e8f0;margin-bottom:12px'></div>",
                      "<table style='width:100%;border-collapse:collapse;margin-bottom:12px'>",
                      "<thead><tr style='background:#2d3748;color:#fff'>",
                      "<th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Name</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Arabic Name</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Category</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Price</th><th style=\"padding:6px 10px;text-align:left;border:0.5px solid #4a5568;font-size:7.5pt\">Unit</th>",
                      "</tr></thead>",
                      `<tbody>${rows}</tbody>`,
                      "</table>",
                      sigHTML,
